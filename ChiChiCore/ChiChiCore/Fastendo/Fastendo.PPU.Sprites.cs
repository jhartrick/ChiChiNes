...
namespace ChiChiNES
{
    public partial class CPU2A03
    {
        ...
        public bool PPU_SpriteCopyHasHappened
        {
        ...
        public int PPU_MaxSpritesPerScanline
        {
        ...
        public void PPU_CopySprites(int copyFrom)
        {
            ...
            for (int i = 0; i < 0x100; ++i)
            {
                ...
                if (spriteRAM[spriteLocation] != Rams[copyFrom + i])
                {
                    spriteRAM[spriteLocation] = Rams[copyFrom + i];
                    unpackedSprites[(spriteLocation / 4) & 0xFF].Changed = true;
                }
            }
        ...
        protected void PPU_InitSprites()
        {
        ...
        public void PPU_PreloadSprites(int scanline)
        {
        ...
        public void PPU_UnpackSprites()
        {
            //Buffer.BlockCopy
            int outBufferloc = 255 * 256;
            for (int i = 0; i < 256; i += 4)
            {
                outBuffer[outBufferloc] = (spriteRAM[i] << 24) | (spriteRAM[i + 1] << 16) | (spriteRAM[i + 2] << 8) | (spriteRAM[i + 3] << 0);
                outBufferloc++;
            }
