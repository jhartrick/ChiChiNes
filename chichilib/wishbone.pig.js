...
Object.defineProperty(exports, "__esModule", { value: true });
var ChiChi_1 = __webpack_require__(6);
var Subject_1 = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"rxjs/Subject\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
var wishbone_tiledoodler_1 = __webpack_require__(9);
var wishbone_cart_1 = __webpack_require__(10);
var WishBopper = /** @class */ (function (_super) {
    __extends(WishBopper, _super);
    function WishBopper() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.audioSettings = new ChiChi_1.AudioSettings();
        _this.EnableNoise = true;
        _this.EnableSquare0 = true;
        _this.EnableSquare1 = true;
        _this.EnableTriangle = true;
        return _this;
    }
    return WishBopper;
}(ChiChi_1.ChiChiBopper));
var WishBoneControlPad = /** @class */ (function () {
    function WishBoneControlPad(machine) {
    ...
    // control pad
    WishBoneControlPad.prototype.handleKeyDownEvent = function (event) {
    ...
    WishBoneControlPad.prototype.handleKeyUpEvent = function (event) {
        switch (event.keyCode) {
            case 37://left arrow
                this.padOneState &= ~64 & 0xFF;
                break;
    ...
    WishBoneControlPad.prototype.refresh = function () {
    };
    WishBoneControlPad.prototype.getByte = function (clock) {
        return 0;
    };
    WishBoneControlPad.prototype.setByte = function (clock, data) {
    };
    return WishBoneControlPad;
}());
exports.WishBoneControlPad = WishBoneControlPad;
var WishbonePPU = /** @class */ (function (_super) {
    __extends(WishbonePPU, _super);
    function WishbonePPU() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    return WishbonePPU;
}(ChiChi_1.ChiChiPPU));
exports.WishbonePPU = WishbonePPU;
var WishboneMachine = /** @class */ (function () {
    function WishboneMachine() {
        var _this = this;
        this.ppuStatus = new ChiChi_1.PpuStatus();
        this.cpuStatus = new ChiChi_1.CpuStatus();
        this.fps = 0;
        this.nesControlBuf = new SharedArrayBuffer(16 * Int32Array.BYTES_PER_ELEMENT);
        this.nesInterop = new Int32Array(this.nesControlBuf);
        this.nesStateSubject = new Subject_1.Subject();
        this.iop_runStatus = 2;
        var wavSharer = new ChiChi_1.WavSharer();
        this.SoundBopper = new WishBopper(wavSharer);
        this.WaveForms = wavSharer;
        this.ppu = new WishbonePPU();
        this.Cpu = new WishboneCPPU(this.SoundBopper, this.ppu);
        this.ppu.cpu = this.Cpu;
        this.Cart = new wishbone_cart_1.WishboneCart();
        this.PadOne = new WishBoneControlPad(this);
        var nesWorker = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"file-loader?name=worker.[hash:20].[ext]!../../../assets/emulator.worker.bootstrap.js\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
        this.worker = new Worker(nesWorker);
        this.worker.onmessage = function (data) {
            _this.handleMessage(data);
        };
    }
    WishboneMachine.prototype.handleMessage = function (data) {
        var d = data.data;
        if (d === 'ready') {
            this.nesReady = true;
    ...
    WishboneMachine.prototype.initChiChiWorker = function () {
    };
    WishboneMachine.prototype.asObservable = function () {
        return this.nesStateSubject.asObservable();
    };
    WishboneMachine.prototype.postNesMessage = function (message) {
        //this.oldOp = this.nesInterop[0] ;
        //this.nesInterop[0] = 0;
        this.worker.postMessage(message);
    };
    WishboneMachine.prototype.Run = function () {
        var _this = this;
        this.postNesMessage({ command: 'run', debug: false });
        setInterval(function () {
            _this.fps = _this.nesInterop[1];
        }, 500);
        // (<any>Atomics).store(this.nesInterop, 2, 1);
    };
    WishboneMachine.prototype.RunFrame = function () {
    };
    WishboneMachine.prototype.RequestSync = function () {
        //case 'audiosettings':
    ...
    WishboneMachine.prototype.Sync = function (data) {
        if (data.bufferupdate) {
            if (data.Cpu.Rams) {
                this.Cpu.Rams = data.Cpu.Rams;
                this.ppu.spriteRAM = data.Cpu.spriteRAM;
                for (var i = 0; i < this.ppu.unpackedSprites.length; ++i) {
    ...
    WishboneMachine.prototype.Drawscreen = function () {
        // throw new Error('Method not implemented.');
    };
    WishboneMachine.prototype.Reset = function () {
        this.postNesMessage({ command: "reset", debug: true });
    };
    WishboneMachine.prototype.PowerOn = function () {
        throw new Error('Method not implemented.');
    };
    WishboneMachine.prototype.PowerOff = function () {
        this.postNesMessage({ command: "stop" });
    };
    WishboneMachine.prototype.Step = function () {
        this.postNesMessage({ command: "step", debug: true });
    };
    WishboneMachine.prototype.EjectCart = function () {
        this.postNesMessage({ command: "stop" });
    };
    WishboneMachine.prototype.LoadCart = function (rom) {
        var cartName = this.Cart.CartName;
        var cart = ChiChi_1.iNESFileHandler.LoadROM(this.Cpu, rom);
        this.Cart = new wishbone_cart_1.WishboneCart();
        this.Cart.realCart = cart;
        this.Cart.realCart.CPU = this.Cpu;
        this.Cart.realCart.Whizzler = this.ppu;
        this.Cart.CartName = cartName;
        this.tileDoodler = new wishbone_tiledoodler_1.TileDoodler(this.Cpu);
        this.postNesMessage({ command: 'loadrom', rom: rom, name: this.Cart.CartName });
        //        this.machine.LoadCart(rom);
    };
    WishboneMachine.prototype.HasState = function (index) {
        throw new Error('Method not implemented.');
    };
    WishboneMachine.prototype.GetState = function (index) {
        throw new Error('Method not implemented.');
    };
    WishboneMachine.prototype.SetState = function (index) {
        throw new Error('Method not implemented.');
    };
    WishboneMachine.prototype.SetupSound = function () {
        throw new Error('Method not implemented.');
    };
    WishboneMachine.prototype.FrameFinished = function () {
        throw new Error('Method not implemented.');
    };
    return WishboneMachine;
}());
exports.WishboneMachine = WishboneMachine;
var WishboneCPPU = /** @class */ (function (_super) {
    __extends(WishboneCPPU, _super);
    function WishboneCPPU(soundBopper, ppu) {
        var _this = _super.call(this, soundBopper, ppu) || this;
        _this.flushHistory = false;
        return _this;
    }
    return WishboneCPPU;
}(ChiChi_1.ChiChiCPPU));
...
Object.defineProperty(exports, "__esModule", { value: true });
var TileDoodler = /** @class */ (function () {
    function TileDoodler(ppu) {
        this.ppu = ppu;
        this.currentNameTableEntries = null;
        this.currentPatternTableEntries = null;
        this.XOffset = 0;
        this.YOffset = 0;
        this.currentPatternTableEntries = new Array(2);
        for (var pt = 0; pt < 2; pt++) {
            this.currentPatternTableEntries[pt] = new Array(32);
            for (var x = 0; x < 32; x++) {
                this.currentPatternTableEntries[pt][x] = new Array(30);
                for (var y = 0; y < 30; y++) {
        ...
        this.currentNameTableEntries = new Array(32);
        for (var i = 0; i < 32; i++) {
            this.currentNameTableEntries[i] = new Array(30);
            for (var j = 0; j < 30; j++) {
    ...
    TileDoodler.prototype.GetPatternTableEntry = function (PatternTable, TileIndex, attributeByte, actualAddress) {
        // 8x8 tile
        ...
        for (var i = 0; i < 8; i++) {
            var entryLoc = (PatternTable + (TileIndex << 4)) + i;
            var patternEntry = this.ppu.ChrRomHandler.GetPPUByte(0, entryLoc);
            actualAddress[i] = this.ppu.ChrRomHandler.ActualChrRomOffset(entryLoc);
            var patternEntryBit2 = this.ppu.ChrRomHandler.GetPPUByte(0, entryLoc + 8);
    ...
    TileDoodler.prototype.GetSprite = function (PatternTable, TileIndex, attributeByte, flipX, flipY) {
        var _this = this;
        // 8x8 tile
        var result = new Uint8Array(64); // System.Array.init(64, 0, System.Int32);
        var yMultiplyer = 8;
        ...
        return result.map(function (val) { return _this.ppu._palette[val]; });
    };
    ...
    //             }
    //         },
    TileDoodler.prototype.DrawRect = function (newData, width, height, xPos, yPos) {
    ...
    //             }
    //         },
    TileDoodler.prototype.DrawAllTiles = function () {
        if (this.YOffset > 256) {
        ...
        var NameTable = 8192 + (1024 * (this.ppu.PPUControlByte0 & 3));
        var nt2 = ((NameTable & 3072) >> 10);
        for (var i = 0; i < 32; i++) {
            for (var j = 0; j < 30; j++) {
                var TileIndex = this.ppu.VidRAM_GetNTByte(8192 + this.ppu.NameTableMemoryStart + i + (j * 32));
                var addToCol = this.GetAttributeTableEntry(this.ppu.NameTableMemoryStart, i, j);
                this.DrawRect(this.GetPatternTableEntry(this.ppu.PatternTableIndex, TileIndex, addToCol, this.currentNameTableEntries[i][j]), 8, 8, (i * 8) + this.XOffset, (j * 8) + this.YOffset);
            }
        }
    };
    TileDoodler.prototype.GetAttributeTableEntry = function (ppuNameTableMemoryStart, i, j) {
        var LookUp = this.ppu.VidRAM_GetNTByte((8192 + ppuNameTableMemoryStart) + 960) + (i >> 2) + ((j >> 2) * 8);
        var AttribByte = 0;
        switch ((i & 2) | ((j & 2) * 2)) {
            case 0:
    ...
    //          * @return  {Array.<number>}
    //          */
    TileDoodler.prototype.DoodleNameTable = function (NameTable) {
        //  a doodle returns an Int32Array containing RGBA values
        var result = new Int32Array(61440);
        //debugger;
        for (var i = 0; i < 32; i = (i + 1) | 0) {
            for (var j = 0; j < 30; j = (j + 1) | 0) {
                var address = ((8192 + NameTable) + i) + (j * 32);
                var TileIndex = this.ppu.ChrRomHandler.GetPPUByte(0, address);
                var addToCol = this.GetAttributeTableEntry(NameTable, i, j);
                var tile = this.GetPatternTableEntry(this.ppu.PatternTableIndex, TileIndex, addToCol, this.currentNameTableEntries[i][j]);
                this.DrawTile(result, 256, 240, tile, (i * 8), (j * 8));
            }
        }
        return result;
    };
    TileDoodler.prototype.DrawTile = function (destBuffer, width, height, tile, xPos, yPos) {
        for (var j = 0; j < 8; j++) {
            for (var i = 0; i < 8; i++) {
                var xPosition = (xPos + 8) - i;
                var yPosition = (yPos + j) * width;
                if (xPos > height) {
                    break;
                }
    ...
    TileDoodler.prototype.GetNameTableEntryLocation = function (x, y) {
        return this.currentNameTableEntries[(((x >> 5)) | 0)][(((y / 30)) | 0)][y & 7];
    };
    TileDoodler.powersOfTwo = [1, 2, 4, 8, 6, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192];
    return TileDoodler;
}());
exports.TileDoodler = TileDoodler;
...
Object.defineProperty(exports, "__esModule", { value: true });
var WishboneCart = /** @class */ (function () {
    function WishboneCart() {
        this.realCart = null;
    }
    Object.defineProperty(WishboneCart.prototype, "NumberOfPrgRoms", {
        get: function () {
    ...
    Object.defineProperty(WishboneCart.prototype, "NumberOfChrRoms", {
        get: function () {
    ...
    Object.defineProperty(WishboneCart.prototype, "MapperID", {
        get: function () {
    ...
    WishboneCart.prototype.LoadiNESCart = function (header, prgRoms, chrRoms, prgRomData, chrRomData, chrRomOffset) {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.InitializeCart = function () {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.UpdateScanlineCounter = function () {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.WriteState = function (state) {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.ReadState = function (state) {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.ActualChrRomOffset = function (address) {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.UpdateBankStartCache = function () {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.ResetBankStartCache = function () {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.GetPPUByte = function (clock, address) {
        if (this.realCart)
            return this.realCart.GetPPUByte(clock, address);
        else
            return 0;
    };
    WishboneCart.prototype.SetPPUByte = function (clock, address, data) {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.GetByte = function (Clock, address) {
        if (this.realCart) {
    ...
    WishboneCart.prototype.SetByte = function (Clock, address, data) {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.HandleEvent = function (Clock) {
        throw new Error('Method not implemented.');
    };
    WishboneCart.prototype.ResetClock = function (Clock) {
        throw new Error('Method not implemented.');
    };
    return WishboneCart;
}());
exports.WishboneCart = WishboneCart;
