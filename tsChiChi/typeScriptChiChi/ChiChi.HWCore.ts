...
class ChiChiInstruction {
        constructor() {
        }

        public AddressingMode: ChiChiCPPU_AddressingModes = ChiChiCPPU_AddressingModes.Bullshit;
        frame: number = 0;
        time: number = 0;
        A: number = 0;
        X: number = 0;
    ...
    class ChiChiCPPU implements ChiChiNES.CPU2A03 {
        ...
        private _instructionHistory: ChiChiInstruction[] = new Array<ChiChiInstruction>(256);//System.Array.init(256, null, ChiChiNES.CPU2A03.Instruction);

        public get InstructionHistory(): Array<ChiChiInstruction> {
        ...
        private spritesOnLine = new Uint32Array(512);// System.Array.init(512, 0, System.Int32);
        ...
        private outBuffer = new Uint32Array(65536);
        ...
        WriteInstructionHistoryAndUsage(): void {
            const newInstruction = new ChiChiInstruction();
            newInstruction.A
        ...
        PPU_GetSpritePixel(): number {
            ...
            for (var i = 0; i < this.spritesOnThisScanline; ++i) {
                const currSprite = this.currentSprites[i];
        ...
        PPU_PreloadSprites(scanline: number): void {
            ...
            for (var spriteNum = 0; spriteNum < 256; spriteNum += 4) {
                let spriteID = ((spriteNum + this._spriteAddress) & 255) >> 2;

                let y = this.unpackedSprites[spriteID].YPosition + 1;
